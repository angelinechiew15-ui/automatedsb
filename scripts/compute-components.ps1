...
SELECT CASE WHEN t.quarter IS NULL THEN t.fy ELSE t.fy || ' ' || t.quarter END AS label,
       ...
       SUM(NVL(TO_NUMBER(ats.cm_matrix_adder_value DEFAULT 0 ON CONVERSION ERROR),0)) AS sum_ats,
       SUM(CAST(CASE WHEN cm_change.cm_matrix_change_value IS NOT NULL THEN TO_NUMBER(cm_change.cm_matrix_change_value DEFAULT 0 ON CONVERSION ERROR) ELSE TO_NUMBER(t."RTU/TS" DEFAULT 0 ON CONVERSION ERROR) END AS BINARY_DOUBLE)) AS sum_rtuts,
...
$r=$cmd.ExecuteReader(); while($r.Read()){ Write-Host "label=$($r['label']) sum_ts=$($r['sum_ts']) sum_ats=$($r['sum_ats']) sum_rtuts=$($r['sum_rtuts']) sum_rtu_adder=$($r['sum_rtu_adder']) sum_costrtu=$($r['sum_costrtu']) sum_dep=$($r['sum_dep']) sum_cost_adder=$($r['sum_cost_adder'])" }
$r.Close(); $conn.Close()